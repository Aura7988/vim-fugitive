...
function M.get_graph(instance_number, start_token, enable_extended_chars, enable_graph, cmd)
  -- Resolve Vim values
  enable_graph = (enable_graph or 0) ~= 0
  enable_extended_chars = (enable_extended_chars or 0) ~= 0
  local enable_dynamic_branch_hl = 1
  ...
  local out_line = 1
  local vim_commits = {}
  local vim_commits_by_hash = { [vim.type_idx] = vim.types.dictionary }
  ...
  for commit_index, commit in ipairs(commits) do
    ...
    local vim_commit = { [vim.type_idx] = vim.types.dictionary }
    ...
    -- Set internal state
    if enable_dynamic_branch_hl then
      local commit_suffix_line = out_line + ncommit_lines
